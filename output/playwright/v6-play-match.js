async (page) => {
  ...
  try {
    ...
    await assertLayout();
    await page.screenshot({ path: `output/playwright/v6-${suffix}-opening.png`, fullPage: true });
    for (let step = 0; step < 250; step += 1) {
      ...
      if (await page.locator('#result-dialog').evaluate((dialog) => dialog.open)
        ...
        count('settlement');
        await page.screenshot({ path: `output/playwright/v6-${suffix}-result.png`, fullPage: true });
      ...
      if (!midgame && counts.attack >= 4 && counts['play-card'] >= 4) {
        await assertLayout();
        await page.screenshot({ path: `output/playwright/v6-${suffix}-midgame.png`, fullPage: true });
        midgame = true;
      }
    ...
    await page.screenshot({ path: `output/playwright/v6-${suffix}-failure.png`, fullPage: true });
    throw error;
  } finally {
