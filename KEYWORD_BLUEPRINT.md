...
更新日期：2026-09-12（补充形态能力与状态展示；下方原 14 关键词蓝图保留历史设计）

## 当前增量

运行目录已包含 26 个关键词，当前实现以 `game-keywords.js` 为准。百闻牌参考表用于设计启发，不能替换本项目既有语义；新增效果应通过注册动作、条件或生命周期实现。

形态能力不是永久被动的第二份拷贝：内容在 `card.formHooks` 中声明，分发时由当前 `unit.form.cardId` 查回目录，复用被动处理器与每回合限次机制；换形态自动停止旧能力。不屈的 `formatUnitStatus` 明确显示生效/待恢复。碎甲是 `remove-shield` 即时动作，晶裂则是伤害增幅层数；`target-frozen` 可用于控制后的条件效果。

角色与 SSR 的当前费用、数量、形态搭配见 [v3 设计](AWAKENING_SSR_BLUEPRINT.md)。存档版本/校验及收藏经济未因本轮改动改变。
...
4. 卡牌进入结算栈；可响应效果把优先权交给对手，响应牌继续压栈，双方连续放弃后才结算当前栈顶。
...
当前 `GAME_STATE_VERSION` 为 v6。运势使用对局自带的确定性 RNG，不能直接调用 `Math.random()`。占卜候选、结算帧、响应优先权、融合数值与幻境实例均可 JSON 序列化。
...
- 响应优先权从栈顶效果控制者的对手开始；打出响应牌后重置连续放弃并转移优先权。
- 第一次放弃只转移优先权，连续第二次放弃结算当前栈顶；下层可响应帧随后重新开启一轮优先权。
- 响应嵌套上限为 `GAME_RULES.maxResponseDepth = 8`，结算栈另有 64 帧安全上限；达到深度上限的响应牌直接结算，不再打开第 9 层窗口。
...
当前自动化测试为 90 项；14 个关键词全部有注册表覆盖，连锁响应、AI 响应决策、幻境目标、v6 状态存档、双方命令重放、按需回放检查点、有限帧缓存和会话存档也有专项回归。
...
- 双方都没有合法响应牌时会自动跳过空响应窗口，不要求玩家进行无意义的两次放弃。
